...
\todoi{some more introducing the sections}
...
\begin{align*}
  &\Fam_s : \Alg_s \to \Type\\
  &\Box_F : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X}) 
    \to F \alg{X} \to \Type\\
  &\Box_G : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X}) 
    (x : F \alg{X}) (y : \Box_F \alg{X} \alg{P}\ x) 
    \to G (\alg{X} , x) \to \Type\\
  &\total : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X}) 
    \to \Alg_s\\
  &\proj : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X}) 
    \to \Alg_s(\total\ \alg{X}\ \alg{P}, \alg{X})\\
  &\phi_F : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X}) 
    \to \Sigma (F \alg{X}) (\Box_F\ \alg{X}\ \alg{P}) \to F (\total\ \alg{X}\ \alg{P})\\
  &\phi_G : (\alg{X} : \Alg_s) (\alg{P} : \Fam_s \alg{X})
    (x : F \alg{X}) (y : \Box_F \alg{X} \alg{P}\ x)
    \to \Sigma (G\ (\alg{X},x)) (\Box_G\ \alg{X}\ \alg{P}\ x\ y) \to G (\total\ \alg{X}\ \alg{P}, \phi_F\ \alg{X}\ \alg{P} (x , y))
\end{align*}
%
