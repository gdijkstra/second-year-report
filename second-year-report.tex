...
We can show that $f\ P \ddefeq (\Sigma X P , \pi_1)$ is an equivalence
(with the preimage family
$\pi^{-1}\ p\ x \ddefeq (a : A) \times (p\ a = x)$ being its
inverse). We can do the following calculation:
%
\begin{align*}
& &&\dephom{\Type} P \\
&=&&(x : X) \to P\ x \\
&=&&(s_0 : X \to X) \times (s_1 : X \to P\ (s_0\ x)) \times (s_0 = \id{X}) \\
&=&&(s : X \to \Sigma X P) \times (\pi_1 \circ s = \id{X}) \\
&=&&\sectionof{\Type} (f\ P)
\end{align*}
%
Generalising this argument to $\algcat{F}$ we need several
ingredients. First of all, we need to show that, given
$\alg{X} : \algcat{F}$,
$\famover{\algcat{F}}{\alg{X}} = \fibover{\algcat{F}}{\alg{X}}$. This
can be done by defining the following functions:
%
\begin{align*}
  &\total_{\alg{X}} : \famover{\algcat{F}}{\alg{X}} \to \algcat{F} \\
  &\total_{\alg{X}} (P,m) \ddefeq (\Sigma X P , \lambda x . (\theta\ (F\ \pi_1\ x) , m\ (\phi_P\ x))) \\
  &\\
  &\proj_{\alg{X}} : (\algfam{P} : \famover{\algcat{F}}{\alg{X}}) \to \algcat{F}(\total_{\alg{X}}\ \algfam{P},\alg{X}) \\
  &\proj_{\alg{X}} (P,m) \ddefeq (\pi_1 , \lambda x . \refl) \\
  &\\
  &\preimage_{\alg{X}} : (\algfam{P} : \fibover{\algcat{F}}{\alg{X}}) \to \famover{\algcat{F}}{\alg{X}} \\
  &\preimage_{\alg{X}} ((A,\alpha),(p,p_0)) \ddefeq (\pi^{-1}\ p , m)
\end{align*}
%
where
$m : (x : FX) \times \Box_F\ (\pi^{-1}\ p)\ x \to \pi^{-1}\ p\
(\theta\ x)$.
This method can be defined in terms of $\alpha$ and $p_0$. It can then
be shown that
$\lambda \algfam{P} . (\total_{\alg{X}}\ \algfam{P},\proj_{\alg{X}}\
\algfam{P})$
and $\preimage_{\alg{X}}$ are inverses of one another, giving us the
desired equivalence.

\todoi{Work out showing that sections of algebra fibrations and
  dependent algebra morphisms are equivalent, generalising the
  situation in $\Type$.}
...
\todoi{Prove that algebra families for general \hits are equivalent to
  algebra fibrations.}
...
\todoi{Generalise dependent algebra morphisms to non-dependent
  dialgebras and show them to be equivalent to the section principle.}
...
\todoi{Work out if we also have completeness for container morphisms,
  \ie for every natural transformation between the extensions of two
  containers, there is a container morphism whose extension is that
...
\todoi{Show that the circle, propositional truncation and
  W-suspensions fit in the above scheme.}
...
\todoi{Explain how the $p_1$ should be the composition of these two
  squares with $f_1\ x$ and $g_1\ x$ as degenerate squares.}

\subsection{Category laws and structure}

\todoi{Specify the coherence needed to show that $\Alg_s$ satisfies
  the identity laws, associativity and has products and equalisers.}
...
\todoi{Spell out the derivation of $\Box_G$ in greater detail.}
...
\todoi{Prove that the dependent algebra morphisms and sections are
  equivalent.}

\subsection{Examples}

\todoi{Show that if we put in the specification of the circle, we get
  something which is equivalent to the usual induction principle of
  the circle.}

\section{Limitations}

\todoi{Work out circle example and show that things do not type check
  as one would hope they would. This is the known issue of using
  W-types to implement inductive types in an intensional type
  theory.}
